...
class Habit(models.Model):
    ...
    place = models.CharField(
        max_length=100, verbose_name="Место", help_text="Место, в котором необходимо выполнять привычку"
    )
    time = models.TimeField(verbose_name="Время", help_text="Время, когда необходимо выполнять привычку")
    action = models.CharField(
        max_length=255, verbose_name="Действие", help_text="Действие, которое представляет собой привычку"
    )
    is_pleasant = models.BooleanField(
        default=False, verbose_name="Приятная привычка", help_text="Признак приятной привычки (способ вознаграждения)"
    )
    ...
    periodicity = models.PositiveSmallIntegerField(
        default=1, verbose_name="Периодичность (дни)", help_text="Периодичность выполнения привычки в днях (от 1 до 7)"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                check=models.Q(duration_to_complete__lte=120), name="duration_to_complete_not_exceed_120"
            ),
            models.CheckConstraint(
                check=models.Q(periodicity__gte=1) & models.Q(periodicity__lte=7), name="periodicity_between_1_and_7"
            ),
        ]
    ...
    def clean(self):
        ...
        validate_pleasant_habit_restrictions(self.is_pleasant, self.reward, self.related_habit)
