...
class TestHabitModel:
    ...
    def test_execution_time_max_120_seconds(self, user):
        ...
        habit = Habit(
            user=user, place="дома", time="09:00", action="читать книгу", periodicity=1, duration_to_complete=121
        )
        with pytest.raises(ValidationError) as exc_info:
            habit.full_clean()
    ...
    def test_periodicity_between_1_and_7(self, user):
        ...
        habit = Habit(
            user=user, place="дома", time="09:00", action="попить воды", periodicity=8, duration_to_complete=30
        )
        with pytest.raises(ValidationError) as exc_info:
            habit.full_clean()
    ...
    def test_periodicity_cannot_be_zero(self, user):
        ...
        habit = Habit(
            user=user, place="дома", time="09:00", action="попить воды", periodicity=0, duration_to_complete=30
        )
