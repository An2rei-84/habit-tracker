...
class TestCeleryTasks:
    ...
    def test_send_habit_reminders(self, mock_timezone, mock_bot_service, db):
        ...
        user = User.objects.create_user(
            username="testuser", email="test@example.com", password="testpass123", telegram_chat_id="123456789"
        )
        ...
        Habit.objects.create(
            user=user, place="дома", time=dt_time(9, 0), action="попить воды", periodicity=1, duration_to_complete=30
        )
    ...
    def test_should_send_reminder_today_created_today(self, user):
        ...
        habit = Habit.objects.create(
            user=user, place="дома", time="09:00", action="попить воды", periodicity=1, duration_to_complete=30
        )
    ...
    def test_should_send_reminder_today_periodicity_7(self, user):
        ...
        habit = Habit.objects.create(
            user=user, place="парк", time="07:00", action="пробежка", periodicity=7, duration_to_complete=120
        )
