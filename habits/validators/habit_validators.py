...
def validate_reward_and_related_habit(reward, related_habit):
    ...
    if reward and related_habit:
        raise ValidationError(
            {
                "non_field_errors": _(
                    "Нельзя указывать и вознаграждение, и связанную привычку одновременно. " "Выберите что-то одно."
                )
            }
        )
...
def validate_execution_time(duration_to_complete):
    ...
    if duration_to_complete and duration_to_complete > 120:
        raise ValidationError(
            {"duration_to_complete": _("Время на выполнение привычки не может превышать 120 секунд.")}
        )
...
def validate_pleasant_habit_restrictions(is_pleasant, reward, related_habit):
    ...
    if is_pleasant and (reward or related_habit):
        raise ValidationError(
            {"non_field_errors": _("У приятной привычки не может быть вознаграждения или связанной привычки.")}
        )
...
def validate_periodicity(periodicity):
    ...
    if periodicity is not None and (periodicity < 1 or periodicity > 7):
        raise ValidationError({"periodicity": _("Периодичность выполнения привычки должна быть от 1 до 7 дней.")})
